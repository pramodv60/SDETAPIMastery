...
public class UpdateUserProfileTest {
	...
	public void updateProfileTest() {
		...
		assertEquals(response.getStatusCode(), 200, "Get Profile should return 200 OK");
		...
		int updateStatus = response.getStatusCode();
		if (updateStatus >= 200 && updateStatus < 300) {
			userResponse = response.as(UserResponse.class);
			assertEquals(userResponse.getAddress(), address, "Address update not successful!");
		}
	}
}